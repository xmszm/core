...
从 v0.0.9 开始，项目实现了完全自动化的 TypeScript 类型生成系统：

**关键特性**：
- ✅ 使用 `vite-plugin-dts` 在构建时自动生成 `.d.ts` 文件
- ✅ 无需手动维护类型定义文件
- ✅ 所有导出的组件、函数、接口自动包含完整类型
- ✅ 支持 Vue 3 SFC 和 JSX/TSX 组件的类型生成

**构建产物**：
- `dist/index.d.ts` - 主库完整类型定义（43KB）
- `dist/plugin/vite/initRouteMeta.d.ts` - 插件类型定义
- `dist/style.d.ts` - 样式文件类型定义

**files 清理**：
- ❌ 删除：`types/index.d.ts`、`types/components.d.ts`、`types/style.d.ts`、`types/plugin/vite/initRouteMeta.d.ts`
- ✅ 保留：`types/auto-imports.d.ts`、`types/src.d.ts`、`types/vue-shim.d.ts`（模块声明）

**发布流程**：
```bash
pnpm run build     # 自动生成 dist 中的所有 .d.ts 文件
npm publish        # 消费者安装时自动获得完整类型定义
```
